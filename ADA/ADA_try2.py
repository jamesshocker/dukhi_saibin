...
data_train = np.loadtxt( 'D:/master_data/training/training.csv', delimiter=',', skiprows=1, converters={32: lambda x:int(x=='s'.encode('utf-8')) } )
...
bdt = AdaBoostClassifier(DecisionTreeClassifier(max_features=12,max_depth= 12,min_samples_split= 3,min_samples_leaf=3),
                         learning_rate = 0.1,
						 algorithm="SAMME",
                         n_estimators= 50)
...
data_test = np.loadtxt( 'D:/master_data/test/test.csv', delimiter=',', skiprows=1 )
...
print 'Writing a final csv file Kaggle_higgs_prediction_output2.csv'
fcsv = open('Kaggle_higgs_prediction_output2.csv','w')
